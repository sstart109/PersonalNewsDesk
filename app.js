    ...
    async function fetchCartoons() {
        ...
        for (let proxy of proxies) {
            try {
                ...
                if (htmlText && htmlText.trim().length > 1000) {
                    if (htmlText.includes('Connection timed out') || htmlText.includes('Forbidden') || htmlText.includes('Server-side requests are not allowed')) {
                        throw new Error('Proxy returned error page');
                    }
                    success = true;
                    console.log(`Successfully fetched cartoons using ${proxy.type}`);
                    break;
                }
            } catch (err) {
                console.warn(`Proxy ${proxy.type} failed for cartoons:`, err);
            }
        }
        ...
        try {
            ...
            console.error('Error parsing cartoons:', error);
        }
